...
// Fetch daily activities for a specific user
export async function getDailyActivities(userId, date) {
  try {
    const [tasks, skills, finance, timeTracking, mood] = await Promise.all([
      supabase
        .from("tasks")
        .select("*")
        .eq("user_id", userId)
...
// Fetch schedule items for a specific user
export async function getScheduleItems(userId) {
  ...
  try {
    ...
    console.error("Error in getScheduleItems:", error)
    throw error
  }
}
...
export async function createScheduleItem(scheduleData) {
  ...
  try {
    ...
    const { data, error } = await supabase.from("schedule").insert([insertData]).select()
    ...
    console.error("Error in createScheduleItem:", error)
    throw error
  }
}
...
export async function updateScheduleItem(itemId, updates) {
  try {
    const { data, error } = await supabase.from("schedule").update(updates).eq("id", itemId).select()
...
export async function deleteScheduleItem(itemId) {
  try {
    const { error } = await supabase.from("schedule").delete().eq("id", itemId)
    if (error) {
...
// Toggle schedule item status
export async function toggleScheduleItemStatus(itemId, currentStatus) {
  ...
  try {
    ...
    if (error) {
      console.error("Error toggling schedule item status:", error)
      throw error
    }

    return data
  } catch (error) {
    console.error("Error in toggleScheduleItemStatus:", error)
    throw error
  }
}

// Get schedule items for a specific day
export async function getScheduleItemsForDay(userId, dayOfWeek) {
  ...
  try {
    ...
    if (error) {
      console.error("Error fetching schedule items for day:", error)
      throw error
    }
...
export function subscribeToScheduleChanges(userId, callback) {
